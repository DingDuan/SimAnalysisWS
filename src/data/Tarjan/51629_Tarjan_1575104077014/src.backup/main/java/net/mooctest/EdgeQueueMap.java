...
class EdgeQueueMap<V> {
	...
	public static class EdgeQueue<V> {
		...
		public void addEdge(ExclusiveEdge<V> exclusiveEdge) {
			...
			if (partition.componentOf(exclusiveEdge.edge.source) == component)
				return;
		...
		public Optional<ExclusiveEdge<V>> popBestEdge(Arborescence<V> bestArborescence) {
			...
			} while (!edges.isEmpty() && edges.comparator().compare(candidates.getFirst(), edges.peek()) == 0 // has to
																												// be
																												// tied
																												// for
																												// best
					&& !bestArborescence.contains(candidates.getFirst().edge)); // break if we've found one in
																				// `bestArborescence`
			// at this point all edges in `candidates` have equal weight, and if one of them
			// is in `bestArborescence`
			// it will be first
			final ExclusiveEdge<V> bestEdge = candidates.removeFirst();
			// edges.addAll(candidates); // add back all the edges we looked at but didn't
			// pick
	...
	public Optional<ExclusiveEdge<V>> popBestEdge(V component, Arborescence<V> best) {
		if (!queueByDestination.containsKey(component))
			return Optional.absent();
	...
	public EdgeQueue merge(V component, Iterable<Pair<EdgeQueue<V>, Weighted<Edge<V>>>> queuesToMerge) {
		...
		for (Pair<EdgeQueue<V>, Weighted<Edge<V>>> queueAndReplace : queuesToMerge) {
			...
			for (ExclusiveEdge<V> wEdgeAndExcluded : queue.edges) {
				...
				result.addEdge(
						ExclusiveEdge.of(wEdgeAndExcluded.edge, replaces, wEdgeAndExcluded.weight - replace.weight));
			}
		}
