 ...
 *
 * This implementation was inspired and informed by those of Nathan L. Fiedler
 ...
 * and Keith Schwarz (htiek@cs.stanford.edu)
 * (http://www.keithschwarz.com/interesting/code/?dir=fibonacci-heap), but
 * rewritten from scratch by Sam Thomson (sthomson@cs.cmu.edu).
 *
 * @param <V>
 *            the type of the values stored in the heap
 * @param <P>
 *            the type of the priorities
 */
public class FibonacciHeap<V, P> implements Iterable<FibonacciHeap<V, P>.Entry> {
	...
	// the largest degree of any root list entry is guaranteed to be <=
	// log_phi(MAX_CAPACITY) = 45
	...
	public void decreasePriority(Entry entry, P newPriority) {
		Preconditions.checkArgument(comparator.compare(newPriority, entry.priority) <= 0, "Cannot increase priority");
	 ...
	 * Returns absent otherwise. No heap consolidation is performed. Runtime: O(1)
	 */
	...
	/**
	 * Returns the entry with the minimum priority, or absent if empty. Runtime:
	 * O(1)
	 */
	 ...
	 * heap to be consolidated, if necessary. Runtime: O(log n) amortized
	 */
	...
	/**
	 * Returns the number of elements in the heap. Runtime: O(1)
	 */
	public int size() {
		return size;
	}
	 ...
	 * the two root lists are just spliced together. Runtime: O(1)
	 */
	...
	/**
	 * Merge two doubly-linked circular lists, given a pointer into each. Return the
	...
	/**
	 * Cuts this entry from its parent and adds it to the root list, and then does
	 * the same for its parent, and so on up the tree. Runtime: O(log n)
	 */
	 ...
	 * there are no more trees of equal degree in the root list. Returns the new
	 * minimum root. Runtime: O(log n) (amortized)
	 */
	private Entry consolidate(Entry someRoot) {
		Entry minRoot = someRoot;
		// `rootsByDegree[d]` will hold the best root we've looked at so far with degree
		// `d`.
		final Object[] rootsByDegree = new Object[MAX_DEGREE];
		for (Entry currRoot : getCycle(someRoot)) {
			// Put `currRoot` into `rootsByDegree`. If there's already something in its
			// spot,
			// merge them into a new tree of degree `degree + 1`. Keep merging until we find
			// an
			// empty spot.
