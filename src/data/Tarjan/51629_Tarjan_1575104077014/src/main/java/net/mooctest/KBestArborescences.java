...
import static net.mooctest.ChuLiuEdmonds.PartialSolution;
import static net.mooctest.Weighted.weighted;
import static java.util.Collections.singleton;

/**
 * Finds the k best arborescences of a complete, directed graph in O(n^2 log n)
 * time. Based on "The k best spanning arborescences of a network." Camerini et
 * al. 1980
 *
...
public class KBestArborescences {
	...
	public static <V> List<Weighted<Arborescence<V>>> getKBestArborescences(WeightedGraph<V> graph, V root, int k) {
		// remove all edges incoming to `root`. resulting arborescence is then forced to
		// be rooted at `root`.
		return getKBestArborescences(graph.filterEdges(not(Edge.hasDestination(root))), k);
	}

	/**
	 * Find the k-best arborescences of `graph`. Equivalent to the RANK function in
	 * Camerini et al. 1980.
	 */
	private static <V> List<Weighted<Arborescence<V>>> getKBestArborescences(WeightedGraph<V> graph, int k) {
		...
		for (int j = 2; j <= k && !queue.isEmpty(); j++) {
			...
			// divide this subset into 2: things that have `edgeToBan`, and those that
			// don't.
			...
			final Weighted<Arborescence<V>> jthBest = ChuLiuEdmonds.getMaxArborescence(graph, item.required, newBanned);
			...
			// subset of solutions in item that *do* have `edgeToBan`, except
			// `bestArborescence`
	...
	static <V> Optional<Weighted<SubsetOfSolutions<V>>> scoreSubsetOfSolutions(WeightedGraph<V> graph,
			Set<Edge<V>> required, Set<Edge<V>> banned, Weighted<Arborescence<V>> wBestArborescence) {
		final WeightedGraph<V> filtered = graph
				.filterEdges(and(not(Edge.competesWith(required)), not(Edge.isIn(banned))));
		final Optional<Pair<Edge<V>, Double>> oEdgeToBanAndDiff = getNextBestArborescence(filtered,
				wBestArborescence.val);
		if (oEdgeToBanAndDiff.isPresent()) {
			final Pair<Edge<V>, Double> edgeToBanAndDiff = oEdgeToBanAndDiff.get();
			return Optional
					.of(weighted(new SubsetOfSolutions<V>(edgeToBanAndDiff.first, wBestArborescence, required, banned),
	...
	/**
	 * Finds the edge you need to ban in order to get the second best solution (and
	 * how much worse that second best solution will be) Corresponds to the NEXT
	 * function in Camerini et al. 1980
	 */
	private static <V> Optional<Pair<Edge<V>, Double>> getNextBestArborescence(WeightedGraph<V> graph,
		...
		final PartialSolution<V> partialSolution = PartialSolution
				.initialize(graph.filterEdges(not(Edge.<V>isAutoCycle())));
		...
		while (!componentsWithNoInEdges.isEmpty()) {
			...
			if (!oMaxInEdge.isPresent())
				continue; // No in-edges left to consider for this component. Done with it!
			...
			if (bestArborescence.parents.get(maxInEdge.edge.destination).equals(maxInEdge.edge.source)) {
				final Optional<ExclusiveEdge<V>> oAlternativeEdge = seek(maxInEdge, bestArborescence,
						partialSolution.unseenIncomingEdges.queueByDestination.get(component));
			...
			if (newComponent.isPresent()) {
				// addEdge created a cycle, which means the new cycle doesn't have any incoming
				// edges
	...
	/**
	 * Determines whether `potentialAncestor` is an ancestor of `node` in
	 * `bestArborescence`
	 */
	...
	/**
	 * Finds `nextBestEdge`, the next best alternative to `maxInEdge` for which the
	 * tail of `maxInEdge` is not an ancestor of the source of `nextBestEdge` in
	 * `bestArborescence`
	 */
	public static <V> Optional<ExclusiveEdge<V>> seek(ExclusiveEdge<V> maxInEdge, Arborescence<V> bestArborescence,
	...
	/**
	 * Represents a subset of all possible arborescences: those that contain all of
	 * `required` and none of `banned`. `bestArborescence` is the best arborescence
	 * in this subset. `weightOfNextBest` is the score of the second best.
	 * `edgeToBan` is the edge you need to ban in order to get the second best (i.e.
	 * the 2nd best in this subset will be the 1st best arborescence with all of
	 * `required`, and none of `banned` U {`edgeToBan`}. The assumption is that
	 * `bestArborescence` has already been added to the k-best list, and we're
	 * trying to decide whether the next best in this subset is k+1-best overall.
	 */
	static class SubsetOfSolutions<V> {
		...
		public SubsetOfSolutions(Edge<V> edgeToBan, Weighted<Arborescence<V>> bestArborescence, Set<Edge<V>> required,
				Set<Edge<V>> banned) {
