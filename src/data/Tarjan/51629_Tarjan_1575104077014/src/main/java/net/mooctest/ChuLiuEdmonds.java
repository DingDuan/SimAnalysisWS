...
import static net.mooctest.EdgeQueueMap.EdgeQueue;
import static net.mooctest.Weighted.weighted;

/**
 * Chu-Liu-Edmonds' algorithm for finding a maximum branching in a complete,
 * directed graph in O(n^2) time. Implementation is based on Tarjan's "Finding
 * Optimum Branchings" paper:
...
public class ChuLiuEdmonds {
	...
	static class PartialSolution<V> {
		...
		// An invariant of the CLE algorithm is that each SCC always has at most one
		// incoming edge.
		// You can think of these edges as implicitly defining a graph with SCCs as
		// nodes.
		...
		// More recently added edges get priority over less recently added edges when
		// reconstructing the final tree.
		private final LinkedList<ExclusiveEdge<V>> edgesAndWhatTheyExclude;
		// a priority queue of incoming edges for each SCC that we haven't chosen an
		// incoming edge for yet.
		final EdgeQueueMap<V> unseenIncomingEdges;
		// running sum of weights.
		// edge weights are adjusted as we go to take into account the fact that we have
		// an extra edge in each cycle
		private double score;

		private PartialSolution(Partition<V> stronglyConnected, Partition<V> weaklyConnected,
				Map<V, Weighted<Edge<V>>> incomingEdgeByScc, LinkedList<ExclusiveEdge<V>> edgesAndWhatTheyExclude,
				EdgeQueueMap<V> unseenIncomingEdges, double score) {
		...
		public static <T> PartialSolution<T> initialize(WeightedGraph<T> graph) {
			...
			return new PartialSolution<T>(stronglyConnected, Partition.singletons(graph.getNodes()), incomingByScc,
					exclusiveEdges, incomingEdges, 0.0);
		}
		...
		/**
		 * Given an edge that completes a cycle, merge all SCCs on that cycle into one
		 * SCC. Returns the new component.
		 */
		private V merge(Weighted<Edge<V>> newEdge, EdgeQueueMap<V> unseenIncomingEdges) {
			// Find edges connecting SCCs on the path from newEdge.destination to
			// newEdge.source
			final List<Weighted<Edge<V>>> cycle = getCycle(newEdge);
			// build up list of queues that need to be merged, with the edge they would
			// exclude
			...
			for (Weighted<Edge<V>> currentEdge : cycle) {
				...
				// if we choose an edge in `queue`, we'll have to throw out `currentEdge` at the
				// end
			...
			// merge the queues and put the merged queue back into our map under the new
			// component
			unseenIncomingEdges.merge(component, queuesToMerge);
			// keep our implicit graph of SCCs up to date:
			// we just created a cycle, so all in-edges have sources inside the new
			// component
			// i.e. there is no edge with source outside component, and destination inside
			// component
		 ...
		 *
		 * Each SCC can only have 1 edge entering it: the edge that we added most
		 * recently. So we work backwards, adding edges unless they conflict with edges
		 * we've already added. Runtime is O(n^2) in the worst case.
		 */
	...
	/**
	 * Find an optimal arborescence of the given graph `graph`, rooted in the given
	 * node `root`.
	 */
	...
	static <V> Weighted<Arborescence<V>> getMaxArborescence(WeightedGraph<V> graph, Set<Edge<V>> required,
			Set<Edge<V>> banned) {
	...
	public static <V> Weighted<Arborescence<V>> getMaxArborescence(WeightedGraph<V> graph) {
		...
		while (!componentsWithNoInEdges.isEmpty()) {
			...
			if (newComponent.isPresent()) {
				// addEdge created a cycle/component, which means the new component doesn't have
				// any incoming edges
				componentsWithNoInEdges.add(newComponent.get());
			}
		}
		// Once no component has incoming edges left to consider, it's time to recover
		// the optimal branching.
