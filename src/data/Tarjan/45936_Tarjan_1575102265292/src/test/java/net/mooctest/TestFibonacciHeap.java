...
//	}
	...
	private void format_print() {
		System.out.printf("==%s==\n", StringEscapeUtils.escapeJava(systemOutRule.getLog()));
	}
	...
	public void testElevenNodeGraph() {
		...
		Weighted w1 = weighted(Edge.from(10).to(3), 3);
		Weighted w2 = weighted(Edge.from(8).to(10), 1);
		assertEquals("Weighted{val=Edge{source=10, destination=3}, weight=3.0}",w1.toString());
		assertEquals("Weighted{val=Edge{source=8, destination=10}, weight=1.0}",w2.toString());
	}
	...
	public void test_my2() {
		...
		for (int i = 0; i < 999; i++) {
			queue.poll();
			assertEquals(i+1,(int)queue.peek());
		}
		queue.poll();
	...
	public void test_my(){
		...
		Pair p3 = new Pair(22,3);
		Pair p4 = new Pair(22,5);
		Pair p5 = new Pair(22,5);
		...
		assertEquals(995,p2.hashCode());
		assertEquals(1646,p3.hashCode());
		assertEquals(1648,p4.hashCode());
		assertEquals(1648,p5.hashCode());

		assertEquals("Pair{first=1, second=2}",p1.toString());
		assertEquals("Pair{first=1, second=3}",p2.toString());
		assertEquals("Pair{first=22, second=3}",p3.toString());
		assertEquals("Pair{first=22, second=5}",p4.toString());
		assertEquals("Pair{first=22, second=5}",p5.toString());

		assertEquals("Arborescence{0 -> 1, 1 -> 2, 2 -> 3}",bestA.val.toString().toString());
		assertEquals("Arborescence{3 -> 1, 1 -> 2, 0 -> 3}",bestB.val.toString().toString());
		assertFalse(bestA.val.equals(null));
		assertTrue(bestA.val.equals(bestA.val));
		assertFalse(bestA.val.equals(bestB.val));
		assertFalse(bestA.val.equals(new Integer(1111)));
	...
	public void test3(){
		...
		for (int i = 0; i < 20; i++) {
			heap.remove(entries.get(120+i));
			assertEquals(50-(i+1),heap.size());
		}
