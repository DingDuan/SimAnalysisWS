...
public interface SimValueModelDao extends JpaRepository<SimValueModel, Integer> {
    ...
    @Query(value = "select * from sim_value_model where (cid1=?1 and cid2=?2) or (cid1=?2 and cid2=?1) and subject=?3", nativeQuery = true)
    ...
    @Query(value = "select * from sim_value_model where subject=?1 and cid1=?2 and cid2=?3", nativeQuery = true)
    List<SimValueModel> searchAllBySubjectAndCids(String subject,int cid1,int cid2);
//
